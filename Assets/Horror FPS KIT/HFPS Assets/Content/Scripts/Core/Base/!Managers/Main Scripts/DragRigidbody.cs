    ...
    public class DragRigidbody : MonoBehaviour
    {
        ...
        void TryDragObject()
        {
            ...
            if (fixedHold)
            {
                ...
                objectHeld.transform.SetParent(scriptManager.ItemPlaceHolder);
                oldScale = heldDraggable.transform.localScale;

                if (heldDraggable.dragScale != heldDraggable.transform.localScale)
                {
                    heldDraggable.transform.localScale = heldDraggable.dragScale;
                }
        ...
        void HoldObject()
        {
           ...
           // distance = Mathf.Clamp(distance, minDistanceZoom, maxDistanceZoom - 0.5f);
            ...
            Vector3 grabPos = scriptManager.ItemPlaceHolder.position + playerAim.direction * distance;
        ...
        public void ResetDrag(bool throwObj)
        {
            ...
            if (objectHeld && heldRigidbody)
            {
                if (fixedHold)
                {
                    objectHeld.transform.parent = oldParent;
                    objectHeld.transform.localScale = oldScale;
                }
